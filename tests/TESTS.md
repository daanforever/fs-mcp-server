...
Функция `view` является алиасом для `read_file` и имеет идентичную функциональность.
...
#### 1. Базовые тесты view (arguments как объект)
- ✓ Чтение обычного файла
- ✓ Чтение многострочного файла
- ✓ Чтение пустого файла
- ✓ Чтение файла с UTF-8
- ✓ Формат ответа (объект с content как массив)

#### 2. Тесты с arguments как строкой JSON
- ✓ Чтение с arguments как строкой
- ✓ Многострочный файл (arguments как строка)
- ✓ Пустой файл (arguments как строка)
- ✓ UTF-8 файл (arguments как строка)

#### 3. Тесты обработки ошибок
- ✓ Несуществующий файл
- ✓ Несуществующий файл (arguments как строка)
- ✓ Отсутствие filename
- ✓ Некорректный JSON в arguments

#### 4. Тесты совместимости с read_file
- ✓ Одинаковый результат view и read_file
- ✓ Одинаковый формат ответа
- ✓ Одинаковая обработка ошибок

#### 5. Тесты вложенных директорий
- ✓ Файл во вложенной директории (объект)
- ✓ Файл во вложенной директории (строка)
...
Тесты проверяют:
- 18 тестовых случаев
- Оба формата передачи аргументов (объект и строка JSON)
- Обработку ошибок
- Полную совместимость с read_file
...
Все 18 тестов для `view` пройдены успешно! ✓
...
- Два формата передачи аргументов для `read_file` и `view` (объект и строка JSON)
- Алиас `view` для `read_file` с идентичной функциональностью
